...
namespace RVO{
    Agent::Agent(const std::string &id, float radius) : maxNeighbors_(20), maxSpeed_(MAX_SPEED), neighborDist_(1000.0f), radius_(radius), timeHorizon_(0.1f), timeHorizonObst_(0.23f), id_(id) {}
    void Agent::prepareStep(const gazebo_msgs::ModelStates &neighbors, const geometry_msgs::Twist &pref)//这里要注意自定义topic来广播neighbors,以及当前无人机向要飞往的速度，与target_pose这个功能生存的cmd_vel类似
    {
        ...
        const size_t neighborCount=neighbors.name.size();//纯获取附近无人机的数量
    ...
    void Agent::insertObstacleNeighbor(Obstacle *obstacle)
	{
		...
		if (distSq < rangeSq) { //插入和排序代码，将边按照与障碍物距离升序排列
	...
	std::string Agent::computeNewVelocity()
	{
		orcaLines_.clear();
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			if (other->id_.substr(0, 3) == "uav") {
				// 是合作无人机
				line.point = velocity_ + 0.5f * u;
			} else {
				// 是非合作障碍物
				line.point = velocity_ + 1.0f * u;
			}
			line.agentID=other->id_;
			orcaLines_.push_back(line);
		}
		...
		if (lineFail < orcaLines_.size()) {
			const Line& blockingLine = orcaLines_[lineFail];
			blockerId = blockingLine.agentID;
			if (!blockingLine.agentID.empty()) {
				std::cout << "UAV " << id_ << ": Blocked by UAV " << blockingLine.agentID << std::endl;
				
			} else {
				std::cout << "UAV " << id_ << ": Blocked by obstacle" << std::endl;
			}
