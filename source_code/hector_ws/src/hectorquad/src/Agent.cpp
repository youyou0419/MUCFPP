...
namespace RVO {
	...
	void Agent::computeNewVelocity()
	{
		...
		for (size_t i = 0; i < obstacleNeighbors_.size(); ++i) {
			...
			const float s = (-relativePosition1 * obstacleVector) / absSq(obstacleVector);//计算智能体到障碍物边界的投影位置，s<0是在后侧，s=0-1是在线段上，s>1是在前侧超过右端点
			const float distSqLine = absSq(-relativePosition1 - s * obstacleVector);//计算智能体到障碍物线段的垂直距离

			Line line;

			if (s < 0.0f && distSq1 <= radiusSq) {
				/* Collision with left vertex. Ignore if non-convex. s<0.0f说明UAV靠近左顶点，distSq1<=radiusSq说明发生了碰撞*/
				if (obstacle1->isConvex_) {  //如果左顶点是凹的说明一定碰撞，但是考虑和它邻近的点就行，不用考虑当前顶点了，如果是凸的就要考虑
			...
			if (s < 0.0f && distSqLine <= radiusSq) {
				...
				const float leg1 = std::sqrt(distSq1 - radiusSq);//暂时没有看懂，一会再看，主要是这个leg1不知道是在干吗
